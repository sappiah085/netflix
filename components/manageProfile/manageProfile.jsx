...
export default function ManageProfileCard({ image, name, handleClick }) {
  return (
    <div className="group flex flex-col items-center gap-2">
      {' '}
      <div
        onClick={handleClick}
        className="w-[100px] relative md:w-[150px] border-zinc-900 hover:border-zinc-300 border-4 cursor-pointer rounded-md flex items-center justify-center aspect-square hover:bg-zinc-300"
      >
        {image && (
        ...
        {image && (
          <span className="absolute bg-black/50 flex items-center justify-center md:text-5xl text-2xl top-0 left-0 w-full h-full text-white">
            <BsPencil />
          </span>
        )}
      </div>
      <span className="group-hover:text-white md:text-2xl text-zinc-400">
        {name}
      </span>
    </div>
  );
